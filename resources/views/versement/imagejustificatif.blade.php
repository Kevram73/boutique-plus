                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-group mt-lg">
                                <div class="card-body col-md-12 col-sm-12">
                                    @php
                                    $fileUrl = asset('storage/'.$depense->justificatif_versement);
                                    $extension = pathinfo($depense->justificatif_versement, PATHINFO_EXTENSION);
                                    @endphp
                                    <p>URL générée : <a href="{{ $fileUrl }}" target="_blank">{{ $fileUrl }}</a></p>
                                    @if (in_array($extension, ['jpg', 'jpeg', 'png', 'gif']))
                                    <img src="{{ $fileUrl }}" alt="Image">
                                    @elseif ($extension === 'pdf')
                                    <iframe style="height: 1000px;" src="{{ $fileUrl }}" width="100%" height="500px"></iframe>
                                    @else
                                    <p>Ce n'est ni une image ni un PDF. Le fichier a l'extension : {{ $extension }}</p>
                                    @endif
                                </div>
                            </div>
